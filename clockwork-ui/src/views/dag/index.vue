<template>
    <div class="app-container">
                    ...
                    <el-row>
                        <el-col :span="8">
                            <el-input class="handle-input" clearable
                                      ...
                                      placeholder="DagID" type="number" v-model="id"></el-input>
                        </el-col>
                        <el-col :span="8">
                            <el-input class="handle-input" clearable
                                      onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))"
                                      placeholder="taskId" type="number" v-model="leaderTaskId"/>
                        </el-col>
                        <el-col :span="8">
                            <el-input class="handle-input" clearable placeholder="taskName" v-model="leaderTaskName"/>
                        </el-col>
                    </el-row>
                </div>
            </div>
            <div class="box_r">
                <el-button @click="refreshHandler" class="fr" icon="el-icon-refresh-left" size="medium" type="success">
                    刷新
                </el-button>
        ...
        <!-- 弹窗：任务详情列表 -->
        <el-dialog :visible.sync="dialogTableVisible" title="任务详情列表" width="75%">
            <el-table :data="taskListData" append border ref="singleTable" style="width: 100%" v-loading="loading">
            ...
            <div class="pagination">
                <el-pagination :current-page="taskListCurrentPage" :page-size="taskListPageSize"
                               :page-sizes="[10, 20, 40, 100]" :total="taskListTotal"
                               @current-change="taskListHandleCurrentChange"
                               @size-change="taskListHandleSizeChange" layout="total, sizes, prev, pager, next, jumper">
                </el-pagination>
            </div>
        </el-dialog>
        <!-- 弹窗：任务详情列表 END -->
        ...
        <!-- 弹窗：任务关系图 -->
        <el-dialog :visible.sync="dialogGraphVisible" title="任务关系图" width="85%">
            <div class="chart" id='dialogChart' style="width: 100%;height: 500px"></div>
        </el-dialog>
        <!-- 弹窗：任务关系图 END -->
        ...
        <el-table :data="tableData" append border ref="singleTable" style="width: 100%" v-loading="loading">
            ...
            <el-table-column label="起始任务ID" prop="leaderTaskId" width="120"></el-table-column>
            <el-table-column label="起始任务名" min-width="180" prop="leaderTaskName"></el-table-column>
            <el-table-column label="任务数" prop="taskCount" width="100">
                <template slot-scope="scope">
                    <p @click="handleTaskList(scope.row.id)" style="color: #2a88bd">{{scope.row.taskCount}}</p>
                </template>
            </el-table-column>
            ...
            <el-table-column fixed="left" label="操作" width="150">
                <template slot-scope="scope">
                    <div class="operate-block">
                        <el-tooltip class="item" content="任务列表" effect="dark" placement="top">
                            <em><i @click="handleTaskList(scope.row.id)" class="el-icon-tickets"></i></em>
                        </el-tooltip>
                        <el-tooltip class="item" content="任务关系图" effect="dark" placement="top">
                            <em><i @click="buildDagGraphForPositionByDagId(scope.row.id)"
                                   class="el-icon-set-up"></i></em>
                        </el-tooltip>
                        <el-tooltip class="item" content="刷新dag信息" effect="dark" placement="top">
                            <em><i @click="refreshDagInfoById(scope.row)" class="el-icon-refresh-left"></i></em>
                        </el-tooltip>
                        <el-popover :popper-options="{ boundariesElement: 'body'}" placement="right" trigger="click"
                            ...
                            <ul class="operate-ul">
                                <li>
                                    <el-popconfirm @onConfirm="rerunTaskByDagId(scope.row.id)" icon="el-icon-info"
                                                   icon-color="red"
                                                   title="你确定要触发该Dag下所有任务吗？">
                                        <i class="el-icon-thumb" slot="reference">触发该所有任务</i>
                                    </el-popconfirm>
                                </li>
                            </ul>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                _self.tableData = [];
                _self.axios.post('/clockwork/web/dag/searchDagPageList', JSON.stringify(pageParam), {
                    headers: {
            ...
            refreshHandler() {
                this.loading = true;
                this.axios.get('/clockwork/web/dag/cleanEmptyDagInfo', null)
                    .then(response => {
                        if (response.code === 'OK') {
                            this.$message({
                                message: '刷新成功',
            ...
            buildDagGraphForPositionByDagId(dagId) {
                ...
                _self.nodes = [];
                _self.links = [];
                _self.axios.get('/clockwork/web/graph/buildDagGraphForPositionByDagId', {
                    params: {
                        'dagId': dagId,
                    ...
                    .then(response => {
                        if (response.code === 'OK') {
                            ...
                            let dialogChart = this.echarts.init(document.getElementById('dialogChart'));
                            dialogChart.clear();
                            _self.$message({
            ...
            refreshDagInfoById(row) {
                this.loading = true;
                this.axios.get('/clockwork/web/dag/refreshDagInfoById', {params: {dagId: row.id}})
                    .then(response => {
                        if (response.code === 'OK') {
                            this.$message({
                                message: '刷新Dag信息成功',
            ...
            rerunTaskByDagId(dagId) {
                let _self = this;
                _self.axios.post('/clockwork/web/task/operation/rerunTaskByDagId', _self.qs.stringify({
                    dagId: dagId,

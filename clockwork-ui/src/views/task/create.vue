    ...
    <div class="app-container">
        ...
        <div class="form-box">
            <el-form :model="form" :rules="rules" label-width="100px" ref="form">
                <el-row>
                    <el-col :span="12">
                        <el-tooltip class="item" content="任务名称仅含有英文字母或数字或下划线，例如：hello_task" effect="dark"
                                    placement="top">
                            <el-form-item label="任务名称" prop="name">
                                <el-input placeholder="任务名称仅含有英文字母或数字或下划线，例如：hello_task" v-model="form.name"></el-input>
                            </el-form-item>
                        </el-tooltip>
                    </el-col>
                    <el-col :span="12">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="脚本名称" prop="scriptName">
                            <el-input @input="(e) => getCommand()" placeholder="请输入脚本名称"
                                      v-model="form.scriptName"></el-input>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="执行命令">
                            <el-input disabled placeholder="脚本目录+脚本名称" v-model="form.command"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="任务组" prop="groupName">
                            <el-autocomplete :fetch-suggestions="querySearchTaskGroup" :on-click="deleteSearchGroup"
                                             @select="handleSelectGroup" class="inline-input"
                    ...
                    <el-col :span="12">
                        <el-form-item label="任务超时时间">
                            <el-input-number :min="1" placeholder="失败重试次数" v-model="form.run_timeout"></el-input-number>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="执行机组" prop="node_gid">
                            <el-select @visible-change="handNodeGidChange" placeholder="请选择任务执行机组" v-model="form.node_gid">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="任务来源" prop="source">
                            <el-select class="handle-input" placeholder="DDS任务，来源选择DDS"
                                       size="medium" v-model="form.source">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="上传脚本">
                            <el-upload :action="uploadFileUrl" :auto-upload="false" :data="form" :file-list="fileList"
                ...
                <!-- dialog 触发方式选项 弹窗 -->
                <el-dialog :before-close="handleClose" :visible.sync="dialogVisible" title="触发方式选项"
                           width="75%">
                    <!-- 时间 -->
                    <el-form-item label="触发时间" v-if="form.triggerMode === '1' || form.triggerMode === '2'">
                            <el-date-picker :clearable='false' :disabled="form.checkPattern"
                                            @change="setTriggerTime"
                                            format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间"
                                            style="width:46%;" type="datetime"
                                            v-model="form.triggerTime"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="执行频率" v-if="form.triggerMode === '1' || form.triggerMode === '2'">
                        <el-row>
                            <el-col :span="11">
                    ...
                    <div v-if="form.triggerMode === '1' || form.triggerMode === '2'">
                        ...
                        <el-form-item label="CRON模式">
                            <el-switch @change="handleCheckPattern" v-model="form.checkPattern"/>
                        </el-form-item>
    ...
    export default {
        data() {
            let nameRule1 = (rule, value, callback) => {
                ...
                if (regExp.test(value) === false) {
                    callback(
                        new Error("任务名称仅能含有英文字母或数字或下划线或.")
                    );
                } else {
                    callback();
                }
        ...
        methods: {
            ...
            getTransferDependencyData() {
                ...
                _self.axios.get("/clockwork/web/task/getTaskIdAndNameByUserName", {
                    params: {
                        userName: userName
                    },
                })
            ...
            handTriggerMode(val) {  // 选择触发模式
                if (!val) {
                    this.dialogVisible = true;
                }
            },
            handNodeGidChange(val) { // 特殊处理
            ...
            submitForm(formName) { // 提交数据
                ...
                if (this.form.name === "" || this.form.location === ""
                    || this.form.scriptName === "" || this.form.triggerMode === ""
                    || this.form.groupName === "" || this.form.source === "") {
                    this.$message({
                        message: "*号必填",
                        type: "warning",
                    });
            ...
            querySearchTaskGroup(queryString, cb) {
                ...
                let results = queryString ? taskGroups.filter(this.createFilterTaskGroup(queryString)) : taskGroups;
            ...
            getUploadPathPrefix() { // 获取上传文件前缀
                this.locationPrefixList = [];
                this.axios.get('/clockwork/web/task/uploadPathPrefix').then((res) => {

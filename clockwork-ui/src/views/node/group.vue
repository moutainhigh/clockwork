<template>
    <div class="app-container">
        <div class="filter-container">
            <div class="form_item">
                <el-input class="handle-input"
                      ...
                      placeholder="节点组ID"
                      size="medium" type="number" v-model="id"/>
            </div>
             <el-button @click="handleCreate" class="fr" icon="el-icon-circle-plus-outline" size="medium" type="primary">添加节点组
            </el-button>
        ...
        <el-dialog :visible.sync="dialogFormVisible" title="添加节点组">
            <el-form :model="nodeGroup" :rules="rules" label-width="100px" ref="node">
                <el-form-item label="节点组名称" prop="name">
                    <el-input placeholder="请输入节点组名称" v-model="nodeGroup.name"></el-input>
                </el-form-item>
                <el-form-item label="节点组描述" prop="description">
                    <el-input placeholder="请输入节点组描述" v-model="nodeGroup.description"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-table :data="tableData" append border ref="singleTable" style="width: 100%" v-loading="loading">
            <el-table-column label="ID" prop="id" sortable width="120"></el-table-column>
            <el-table-column label="组名称" prop="name" width="180">
                <template slot-scope="scope">
                    <template v-if="scope.row.edit">
                        <el-input class="edit-input" size="small" v-model="scope.row.name"/>
                    </template>
            ...
            <el-table-column label="描述" min-width="250" prop="description">
                <template slot-scope="scope">
                    <template v-if="scope.row.edit">
                        <el-input class="edit-input" size="small" v-model="scope.row.description"/>
                    </template>
                ...
                <template slot-scope="scope">
                    <div class="operate-block">
                        ...
                        <el-tooltip class="item" content="修改" effect="dark" placement="top" slot="reference"
                                    v-if="!scope.row.edit">
                            <em><i @click="scope.row.edit=!scope.row.edit" class="el-icon-edit"/></em>
                        </el-tooltip>

                        <el-tooltip class="item" content="确认" effect="dark" placement="top" slot="reference"
                                    v-if="scope.row.edit">
                            <em><i @click="confirmEdit(scope.row)" class="el-icon-check"/></em>
                        </el-tooltip>

                        <el-tooltip class="item" content="取消" effect="dark" placement="top" slot="reference"
                                    v-if="scope.row.edit">
                            <em><i @click="cancelEdit(scope.row)" class="el-icon-close"/></em>
                        </el-tooltip>
                    </div>
                </template>
            </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                ...
                _self.tableData = [];
                _self.axios.post('/clockwork/web/node/group/searchNodeGroupPageList', JSON.stringify(pageParam), {
                    headers: {
            ...
            confirmEdit(row) {
                row.edit = false;
                let _self = this;
                _self.axios.post('/clockwork/web/node/group/updateNodeGroup', JSON.stringify(row), {
            ...
            submitCreate(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        _self.axios.post('/clockwork/web/node/group/addNodeGroup', JSON.stringify(this.nodeGroup), {
                            headers: {'Content-Type': 'application/json'}
                        })
                            .then(response => {
                                if (response.code === 'OK') {
            ...
            handleDelete(status, row) {
                let id = row.id;
                let _self = this;
                _self.axios.post('/clockwork/web/node/group/deleteNodeGroup', _self.qs.stringify({id: id}))
                    .then(response => {
                        if (response.code === 'OK') {
                            _self.$message({
                                message: '操作成功',
